...
const Message = ({ data }) => {
  return (
    <div className="p-8 rounded-3xl bg-white/5 border border-white/10 hover:border-purple-500/30 transition-all hover:translate-y-[-4px] group">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h4 className="font-bold text-white group-hover:text-purple-400 transition-colors">{data?.name || "Anonymous"}</h4>
          <p className="text-xs text-slate-500">{data?.date || "Recently"}</p>
        </div>
      </div>
